...
The reason why the lifecycle is split for these two types is historical experience managing these two concepts. Based on
what we learned in the past years, it turned out that it's much easier to reason about the lifecycle of Entities, and their
fields (composite, or primitive) this way.

The recommended way of looking at this is:
