...
Every static query defined in our model will manifest a dedicated DAO with a single method.

Besides the above, Static Queries behave the exact same way as Instance Queries do.
...
In this example we can see that the modeled `totalNumberOfLeads` query resulted in the `getTotalNumberOfLeads()` method
being added to the dedicated `TotalNumberOfLeads.TotalNumberOfLeadsDao`.