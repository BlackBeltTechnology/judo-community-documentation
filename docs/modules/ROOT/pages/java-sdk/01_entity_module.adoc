...
You can influence the behavior of a field or a relation. You have the flexibility to fetch a field like a relation, or vice versa.
...
entity OrderItem {
	...
	field required Integer amount;
	field String productName <= self.product.name eager: false;
...
public void testFetch() {
    ...
    assertEquals("Butter",orderItemDao.queryProductName(orderItem).get());
...
Parameterless static queries are generated on their corresponding dedicated DAOs and contains an `.execute()` method.
...
* Collections: start with "query", and parameters can be set on it. After that you must trigger with the `.execute()` method.
* Single references, or primitives: contains an `.execute()`, and parameters can be set on the same method.
